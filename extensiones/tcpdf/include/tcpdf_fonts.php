...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (empty($fonttype)) {
			if (TCPDF_STATIC::_getULONG($font, 0) == 0x1000) {
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x1000) {
				// sfnt version must be 0x0001000 for TrueType version 1.0.
				return false;
			}
							...
							for ($i = 0; $i < $nGroups; ++$i) {
								...
								for ($k = $startCharCode; $k <= $endCharCode; ++$k) {
									...
									if ((isset($is32[$is32idx])) AND (($is32[$is32idx] & (1 << (7 - ($c % 8)))) == 0)) {
										...
										//LEAD_OFFSET = (0xD800 - (0x1000 >> 10)) = 55232
										//SURROGATE_OFFSET = (0x1000 - (0xD800 << 10) - 0xDC00) = -56613888
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		// rebuild font
		$font = '';
		$font .= pack('N', 0x1000); // sfnt version
	 ...
	 *    than 0x10FFFF.
	 *
	 *    1) If U < 0x1000, encode U as a 16-bit unsigned integer and
	 *       terminate.
	 *
	 *    2) Let U' = U - 0x1000. Because U is less than or equal to 0x10FFFF,
	 ...
	 *       (hexadecimal)    |              (binary)
	 *    --------------------+-----------------------------------------------
	 *    000 000-000 007F | 0xxxxxxx
	 *    000 0080-000 07FF | 110xxxxx 10xxxxxx
	 *    000 0800-000 FFFF | 1110xxxx 10xxxxxx 10xxxxxx
	 *    0001 000-0010 FFFF | 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
	 *    ---------------------------------------------------------------------
	 *
	 *   ABFN notation:
	 *   ---------------------------------------------------------------------
